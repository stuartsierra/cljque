...
(defn subscribe
  "Subscribes to messages.  With two arguments, invokes (f message) on
  each messaged generated by observable and discards f's return value.

  With three or four arguments, calls (send agnt on-message message)
  for each message generated by observable.  With four arguments,
  additionally calls (send agnt on-done) when the observable is
                     ...
                     (subscribe
                      ...
                      (fn [state message]
                        (let [new-state (assoc state i message)]
                          (if (every? #(not= ::unset %) new-state)
                            (do (doseq [u @unsubs] (u))
                                (send agnt on-message new-state)
                                (conj new-state ::unset))
                            new-state)))
                      (fn [state]
...
(defn delay
  "For each message generated by source, generates an identical
  message d units later."
  [d units source]
  (reify Observable
    (observe [this agnt on-message on-done]
      (subscribe source
                 (make-agent nil)
                 (static-receiver #(delay-send d units agnt on-message %))
                 (fn [state] (delay-send d units agnt on-done) state)))))

(defn periodic-repeat [init d units message]
  (reify Observable
    (observe [this agnt on-message on-done]
      (periodic-send init d units agnt on-message message))))

(defn periodic-seq
  "After an initial delay of init units, then once every d units,
  generate a message from the next element of sequence s, which should
  not block."
  [init d units s]
  (reify Observable
    (observe [this agnt on-message on-done]
      (let [unsub (promise)]
        (deliver unsub
                 (periodic-send init d units (make-agent (seq s))
                                (fn [state]
                                  (if-let [x (clojure.core/first state)]
                                    (do (send agnt on-message x)
                                        (clojure.core/next state))
                                    (do (@unsub)
                                        (send agnt on-done)
...
(defn envelope [source]
  (map (fn [message] {:source source :message message}) source))

(defn first-in
  "Forwards messages from whichever source is the first to generate a
  message, and only that source."
  [& sources]
  (reify Observable
    (observe [this agnt on-message on-done]
      (let [unsubs (promise)
...
(defn timeout
  "Forwards messages from source.  If source does not generate at
  least one message message within d units, generates
  message ::timeout."
  [d units source]
  (first-in (delay d units (value :timeout)) source))
